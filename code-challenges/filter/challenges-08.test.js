...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(element => forbiddenValues.indexOf(element) < 0)
};
...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(stats => stats.baseStat > minBaseStat).map(attr => attr.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(kids => !kids.hasOwnProperty('children'));
};
